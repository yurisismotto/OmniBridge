package io.github.yurisismotto.pliwee.identity
...
class DeviceIdentity private constructor(
    ...
    companion object {
        ...
        /**
         * The Pliwee identity alias.
         *
         * The alias namespace restarts at v1 with the Pliwee applicationId
         * (ADR-0020 §D1), as it did for OmniBridge (ADR-0018). It is not a
         * continuation of `omnibridge-identity-v1` or of the AnyFlow-era
         * `anyflow-identity-v{1,2}` aliases and it cannot be one: those lived
         * in the Android Keystore of `io.github.yurisismotto.omnibridge` and
         * `io.github.yurisismotto.anyflow`, and an app cannot reach another
         * app's keystore entries. Nothing is migrated and nothing is
         * destroyed — a fresh Pliwee install simply generates its own key,
         * and the old app keeps its own until it is uninstalled.
         *
         * The consequence is a one-time re-pair: the desktop meets the phone
         * as a new device (ADR-0020 §D12, peer-side boundary).
         */
        private const val KEY_ALIAS = "pliwee-identity-v1"
        ...
        private fun generate(deviceId: String): DeviceIdentity {
            // No legacy-alias sweep (ADR-0020 §D1). `omnibridge-*` and
            // `anyflow-*` entries belong to other applicationIds and are not
            // reachable from this one; [KEY_ALIAS] is the only identity alias
