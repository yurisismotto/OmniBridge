...
//!
//! The policy *type* lives in [`pliwee_core::clipboard_policy`], because it
...
pub use pliwee_core::clipboard_policy::ClipboardPolicy;
...
pub trait ClipboardAuthorizer: Send + Sync {
    ...
    /// forget the other.
    async fn policy_for(&self, peer: &pliwee_core::Fingerprint) -> ClipboardPolicy;
    ...
    async fn auto_send_peers(&self) -> Vec<pliwee_core::Fingerprint>;
}
