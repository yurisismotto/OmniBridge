        ...
        match self {
            ...
            Self::TimedOut => f.write_str(
                "the clipboard did not respond in time: wl-copy and wl-paste \
                 ...
                 `pliwee clipboard status` to see which applies here.",
            ),
        }
    }
}
...
pub struct ClipboardWatch {
    ...
    /// What is doing the watching, for `pliwee clipboard status`.
...
pub trait ClipboardBackend: Send + Sync {
    ...
    ///
    /// A pure predicate answered from what was probed at startup: it must not
    /// perform I/O and must not start a watcher. `pliwee clipboard status`
    ...
    /// `pliwee clipboard status`. Must not perform I/O.
...
/// with an honest `FAILED`, and still report *why* in `pliwee clipboard
