...
//!
//! ```console
//! cargo test -p pliwee-capability-clipboard --test real_backend -- --ignored --test-threads=1
//! ```
//!
...
use pliwee_capability_clipboard::backend::{self, BackendError, ClipboardBackend};
...
async fn the_real_clipboard_carries_a_maximum_sized_clip() {
    preserving_clipboard(|b| async move {
        let big = "x".repeat(pliwee_capability_clipboard::limits::MAX_CLIPBOARD_TEXT_BYTES);
        ...
        assert_eq!(
            read_back.len(),
            pliwee_capability_clipboard::limits::MAX_CLIPBOARD_TEXT_BYTES,
...
async fn a_sensitive_write_honours_this_backend_s_advertised_capability() {
    ...
    const CANARY: &str = "pliwee-sensitive-canary-3f9c1a";
