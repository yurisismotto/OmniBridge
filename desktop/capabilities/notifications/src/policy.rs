...
//!
//! The policy *type* lives in [`pliwee_core::notification_policy`], because
...
pub use pliwee_core::notification_policy::{LockPolicy, NotificationPolicy};
...
pub trait NotificationAuthorizer: Send + Sync {
    ...
    async fn policy_for(&self, peer: &pliwee_core::Fingerprint) -> NotificationPolicy;
}
...
impl NotificationAuthorizer for DenyAll {
    async fn policy_for(&self, _peer: &pliwee_core::Fingerprint) -> NotificationPolicy {
