        ...
        tokio::spawn(async move {
            if let Err(e) = activator.activate(action, token).await {
                ...
                tracing::info!(
                    ...
                    reason = %e,
                    "could not present the Pliwee window the tray asked for"
                );
            }
        });
    }
...
impl StatusNotifierItem {
    ...
    /// without Pliwee knowing anything about the display. Sending pixels
    ...
    /// a path that means nothing on the other side. Pliwee installs its icon
    ...
    /// primary action and that a left click should just open the menu. Pliwee
    ...
    /// continuous quantity. Pliwee has no such quantity: everything it could
