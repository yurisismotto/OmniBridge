        ...
        tokio::spawn(async move {
            ...
            if let Err(e) = activator.activate(action, None).await {
                tracing::info!(
                    ...
                    reason = %e,
                    "could not present the Pliwee window the tray menu asked for"
                );
            }
        });
    }
...
fn entry_properties(entry: &model::MenuEntry) -> HashMap<String, OwnedValue> {
    ...
    // something Pliwee can always do — if the GUI is not running, the bus
...
impl TrayMenu {
    ...
    /// refused here for the same reason: nothing Pliwee does is an
    /// interruption.
    #[zbus(property)]
    fn status(&self) -> &str {
        "normal"
