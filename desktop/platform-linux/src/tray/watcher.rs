...
//!   a duplicate would show two Pliwee icons.
//!
...
pub async fn follow(connection: &zbus::Connection, item_bus_name: &str) -> zbus::Result<()> {
    ...
    match dbus.get_name_owner(WATCHER_BUS_NAME.try_into()?).await {
        ...
        Err(_) => {
            ...
            tracing::info!(
                "no desktop tray host on this session; the Pliwee tray item is \
        ...
        match args.new_owner().as_ref() {
            ...
            None => {
                if registered_with.take().is_some() {
                    ...
                    // at costs nothing.
                    tracing::info!(
                        "the desktop tray host went away; the Pliwee tray item stays \
...
async fn register(connection: &zbus::Connection, item_bus_name: &str) -> bool {
    ...
    match proxy.call_method(REGISTER_METHOD, &(item_bus_name,)).await {
        Ok(_) => {
            tracing::info!(item = %item_bus_name, "registered a Pliwee tray item with the desktop shell");
            true
        }
        Err(e) => {
            ...
            tracing::info!(
                ...
                "the desktop tray host refused the Pliwee tray item"
            );
            false
        }
    }
