...
const PROTO_FILE: &str = "pliwee/v1/capabilities/notifications_v1.proto";
...
/// width enforced in `pliwee_core::notifications`.
#[test]
    ...
    for m in messages() {
        ...
        for f in &m.field {
            if f.r#type() == Type::Message {
                let ty = f.type_name();
                assert!(
                    ty.starts_with(".pliwee.v1.capabilities."),
...
fn the_new_schema_changes_no_existing_file() {
    ...
    let file = &set.file[0];
    assert_eq!(file.package(), "pliwee.v1.capabilities");
